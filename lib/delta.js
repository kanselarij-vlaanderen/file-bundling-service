...
async function handleFileDeletions (deletedFiles) {
  for (var deletedFile of deletedFiles) {
    ...
    const jobsWithoutMembers = await findJobsWithoutCollectionMembers();
    const jobsWithoutArchive = await findFinishedJobsWithoutPhysicalFile(); // no 'job.physf' available
    const jobs = [...new Set([...jobsFromDelta, ...jobsWithoutMembers, ...jobsWithoutArchive])]; // Uniquify array
